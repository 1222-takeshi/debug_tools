...
class BagFileParser():
    ...
    def run(self):
        for topic, name in zip(self.topics, self.topic_names):
            stamp_list = []
            for msg in topic:
                stamp = msg[0] / 1000000000 if self.stamp_type == "real" else msg[1].header.stamp.sec + \
                    msg[1].header.stamp.nanosec / 1000000000
        ...
        ax.minorticks_on()
        ax.grid(which="major", axis="x", color="black", alpha=0.8,
                linestyle="--", linewidth=1)
        ax.grid(which="major", axis="y", color="gray", alpha=0.8,
                linestyle="--", linewidth=1)
        ax.grid(which="minor", axis="y", color="gray", alpha=0.8,
                linestyle="--", linewidth=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--stamp_type', '-s', default="real", type=str,
                        choices=["real", "ideal"], help='real or ideal')
